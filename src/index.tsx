...
type Variables = { userId: number; userName: string; userRole: string; userSubscription: string; userEmail: string; subscriptionStatus: string }
...
function getMonthlyOrderLimit(subscription: string): number {
  const s = (subscription || 'essentiel').toLowerCase()
  if (s === 'essentiel' || s === 'starter') return 300
  if (s === 'pro') return 1000
  if (s === 'elite' || s === 'business') return 7000
  return 300
}
...
app.use('/api/*', async (c, next) => {
  ...
  const session = await c.env.DB.prepare(
    "SELECT s.user_id, u.username, u.nom, u.prenom, u.role, u.subscription, u.email, u.trial_expires_at, u.subscription_status, u.subscription_expires_at FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ? AND s.expires_at > datetime('now')"
  ...
  // Auto-update subscription_status si expiré
  let subStatus = session.subscription_status || 'trial'
  if (session.role !== 'admin') {
    const now = new Date()
    if (subStatus === 'trial' && session.trial_expires_at && new Date(session.trial_expires_at) < now) {
      subStatus = 'expired'
      await c.env.DB.prepare("UPDATE users SET subscription_status = 'expired' WHERE id = ?").bind(session.user_id).run()
    } else if (subStatus === 'active' && session.subscription_expires_at && new Date(session.subscription_expires_at) < now) {
      subStatus = 'expired'
      await c.env.DB.prepare("UPDATE users SET subscription_status = 'expired' WHERE id = ?").bind(session.user_id).run()
    }
  }
  ...
  c.set('subscriptionStatus', subStatus)
  return next()
})
...
app.use('/api/*', async (c, next) => {
  const path = new URL(c.req.url).pathname
  // Ignorer les routes publiques (déjà gérées par le middleware auth)
  if (path.startsWith('/api/auth/')) return next()
  const role = c.get('userRole')
  // role admin bypass toujours
  if (role === 'admin') return next()
  const status = c.get('subscriptionStatus')
  if (status === 'expired' || status === 'suspended') {
    // Autoriser uniquement les routes liées à la tarification/paiement
    const allowedPaths = ['/api/payment-request', '/api/admin/payment-requests']
    const isAllowed = allowedPaths.some(p => path === p || path.startsWith(p + '/'))
    if (!isAllowed) {
      return c.json({ error: 'Abonnement expiré. Choisissez un plan pour continuer.', code: 'SUBSCRIPTION_EXPIRED' }, 403)
    }
  }
  return next()
})
...
app.post('/api/auth/register', async (c) => {
  ...
  // Insérer l'utilisateur avec abonnement par défaut + période d'essai 7 jours
  const result = await c.env.DB.prepare(
    `INSERT INTO users (username, password_hash, prenom, nom, email, telephone, store_name, role, active, subscription, subscription_status, trial_expires_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 'client', 1, 'starter', 'trial', datetime('now', '+7 days'))`
...
app.get('/api/auth/check', async (c) => {
  ...
  const session = await c.env.DB.prepare(
    "SELECT s.user_id, u.username, u.nom, u.prenom, u.role, u.store_name, u.subscription, u.email, u.trial_expires_at, u.subscription_status, u.subscription_expires_at FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ? AND s.expires_at > datetime('now')"
  ).bind(token).first() as any
  ...
  return c.json({
    ...
    user: {
      ...
      role: session.role,
      subscription: session.subscription || 'starter',
      store_name: session.store_name,
      email: session.email || '',
      subscription_status: session.role === 'admin' ? 'active' : subStatus,
      trial_expires_at: session.trial_expires_at || null,
...
app.get('/api/admin/users', async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT id, username, email, prenom, nom, telephone, store_name, role, active, created_at, last_login,
            subscription, subscription_status, trial_expires_at, subscription_expires_at
...
// ========================
app.post('/api/admin/users/:id/subscription', async (c) => {
  const id = Number(c.req.param('id'))
  const adminId = c.get('userId')
  ...
  switch (action) {
    case 'activate_30d':
      updateSQL = "UPDATE users SET subscription_status = 'active', subscription_expires_at = datetime('now', '+30 days') WHERE id = ?"
      logDetails = `activate_30d on user ${id}`
      break
    case 'activate_1y':
      updateSQL = "UPDATE users SET subscription_status = 'active', subscription_expires_at = datetime('now', '+365 days') WHERE id = ?"
      logDetails = `activate_1y on user ${id}`
      break
    case 'suspend':
      updateSQL = "UPDATE users SET subscription_status = 'suspended' WHERE id = ?"
      logDetails = `suspend user ${id}`
      break
    case 'trial_7d':
      updateSQL = "UPDATE users SET subscription_status = 'trial', trial_expires_at = datetime('now', '+7 days') WHERE id = ?"
      logDetails = `trial_7d on user ${id}`
      break
    default:
  ...
  await c.env.DB.prepare('INSERT INTO historique (action, details, commande_id) VALUES (?, ?, ?)').bind(
    'ADMIN_SUBSCRIPTION',
    JSON.stringify({ admin_id: adminId, target_id: id, action: logDetails }),
    null
  ).run()
...
app.get('/api/revenue', async (c) => {
  ...
  const isAdmin = c.get('userRole') === 'admin'
  const hasSuiviUserId = await hasTableColumn(c.env.DB, 'suivi', 'user_id')
  const scope = !isAdmin && hasSuiviUserId ? ' AND user_id = ?' : ''
  const scopeWhere = !isAdmin && hasSuiviUserId ? ' WHERE user_id = ?' : ''
  const params = !isAdmin && hasSuiviUserId ? [userId] : []
  const paramsAnd = !isAdmin && hasSuiviUserId ? [userId] : []

  const livreFilter = `UPPER(statut) LIKE '%LIVR%'`

  const [caTotal, caCeMois, totalLivre, livresCeMois, caParMois, topProduits, caParTransporteur] = await Promise.all([
    // CA total toutes livraisons
    c.env.DB.prepare(`SELECT SUM(prix) as total FROM suivi WHERE ${livreFilter}${scope}`).bind(...paramsAnd).first(),
    // CA ce mois
    c.env.DB.prepare(`SELECT SUM(prix) as total FROM suivi WHERE ${livreFilter} AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')${scope}`).bind(...paramsAnd).first(),
    // Total livraisons
    c.env.DB.prepare(`SELECT COUNT(*) as c FROM suivi WHERE ${livreFilter}${scope}`).bind(...paramsAnd).first(),
    // Livraisons ce mois
    c.env.DB.prepare(`SELECT COUNT(*) as c FROM suivi WHERE ${livreFilter} AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')${scope}`).bind(...paramsAnd).first(),
    // CA par mois (6 derniers mois)
    c.env.DB.prepare(
      `SELECT strftime('%Y-%m', created_at) as mois,
              COUNT(*) as nb_livres,
              SUM(prix) as ca,
              SUM(prix - CASE WHEN LOWER(livraison) LIKE '%stop%' THEN 450 ELSE 650 END) as marge
       FROM suivi
       WHERE ${livreFilter} AND created_at >= date('now', '-6 months')${scope}
       GROUP BY mois
       ORDER BY mois DESC
       LIMIT 6`
    ).bind(...paramsAnd).all(),
    // Top 8 produits par CA
    c.env.DB.prepare(
      `SELECT produit,
              COUNT(*) as nb_ventes,
              SUM(prix) as ca,
              SUM(prix - CASE WHEN LOWER(livraison) LIKE '%stop%' THEN 450 ELSE 650 END) as marge
       FROM suivi WHERE ${livreFilter}${scope}
       GROUP BY produit
       ORDER BY ca DESC
       LIMIT 8`
    ).bind(...paramsAnd).all(),
    // CA par transporteur
    c.env.DB.prepare(
      `SELECT transporteur, COUNT(*) as nb, SUM(prix) as ca
       FROM suivi WHERE ${livreFilter}${scope}
  ...
  const caTotalVal = Number((caTotal as any)?.total || 0)
  const livreTotal = Number((totalLivre as any)?.c || 0)
  const panierMoyen = livreTotal > 0 ? Math.round(caTotalVal / livreTotal) : 0

  // Calculer total commandes ce mois (livrees + en cours) pour taux
  const [totalCeMois] = await Promise.all([
    c.env.DB.prepare(`SELECT COUNT(*) as c FROM suivi WHERE strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')${scope}`).bind(...paramsAnd).first()
  ])
  const totalCeMoisVal = Number((totalCeMois as any)?.c || 0)
  const livresCeMoisVal = Number((livresCeMois as any)?.c || 0)
  const tauxCeMois = totalCeMoisVal > 0 ? Math.round((livresCeMoisVal / totalCeMoisVal) * 100) : 0

  return c.json({
    ca_total: caTotalVal,
    ca_ce_mois: Number((caCeMois as any)?.total || 0),
    total_livraisons: livreTotal,
    livraisons_ce_mois: livresCeMoisVal,
    panier_moyen: panierMoyen,
    taux_livraison_ce_mois: tauxCeMois,
    ca_par_mois: (caParMois as any)?.results || [],
    top_produits: (topProduits as any)?.results || [],
    ca_par_transporteur: (caParTransporteur as any)?.results || [],
  })
})
...
function traduireStatutZR(stateName: string): string {
  ...
  const mapping: Record<string, string> = {
    ...
    retour: '↩️ Retour Expéditeur', returned: '↩️ Retour Expéditeur', retourne: '↩️ Retour Expéditeur', echec: '↩️ Retour Expéditeur',
...
function traduireStatutYalidine(status: any): string {
  ...
  const mapping: Record<string, string> = {
    ...
    '11': '🚚 En cours de livraison', '12': '✅ Livré & Encaissé', '13': 'ECHEC LIVRAISON', '14': '↩️ Retour Expéditeur', '15': '↩️ Retour Expéditeur'
  }
  if (mapping[s]) return mapping[s]

  const text = s.toLowerCase()
  if (text.includes('livre')) return '✅ Livré & Encaissé'
  if (text.includes('retour')) return '↩️ Retour Expéditeur'
...
function traduireStatutEcotrack(status: any): string {
  ...
  const mapping: Record<string, string> = {
    ...
    'en livraison': '🚚 En cours de livraison', 'livre': '✅ Livré & Encaissé', 'echoue': '↩️ Retour Expéditeur',
    'retourne': '↩️ Retour Expéditeur', 'annule': 'Annule'
  }
  if (mapping[s]) return mapping[s]
  for (const [key, value] of Object.entries(mapping)) { if (s.includes(key)) return value }
  return s.toUpperCase()
